...
class DecoupledMVRowSelfAttnProcessor2_0(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.use_lora != False:
            self.lora_q_A = nn.Linear(inner_dim, 32)
            self.lora_k_A = nn.Linear(inner_dim, 32)
            self.lora_v_A = nn.Linear(inner_dim, 32)
            
            self.lora_q_B = nn.ModuleList([nn.Linear(32, inner_dim) for _ in range(6)])
            for linear in self.lora_q_B:
                nn.init.constant_(linear.weight, 0.0)
                if linear.bias is not None:
                    nn.init.constant_(linear.bias, 0.0)
                    
            self.lora_k_B = nn.ModuleList([nn.Linear(32, inner_dim) for _ in range(6)])
            for linear in self.lora_k_B:
                nn.init.constant_(linear.weight, 0.0)
                if linear.bias is not None:
                    nn.init.constant_(linear.bias, 0.0)
                    
            self.lora_v_B = nn.ModuleList([nn.Linear(32, inner_dim) for _ in range(6)])
            for linear in self.lora_v_B:
                nn.init.constant_(linear.weight, 0.0)
                if linear.bias is not None:
                    nn.init.constant_(linear.bias, 0.0)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if use_mv:
            batches = hidden_states.size(0) // 6
            parts = []
            if batches == 1:
                for i in range(6):
                    copy_layer_name = f"to_q_mv_copy_{i}" if i > 0 else "to_q_mv"
                    if self.use_lora != False:
                        part = hidden_states[0:1]
                        if i==0:
                            parts.append(getattr(self, copy_layer_name)(part)+0.5*self.lora_q_B[i](self.lora_q_A(part)))
                        else:
                            parts.append(getattr(self, copy_layer_name)(hidden_states[i:i+1])+0.5*self.lora_q_B[i](self.lora_q_A(part)))
                    else:
                        parts.append(getattr(self, copy_layer_name)(hidden_states[i:i+1]))
            else:
                for i in range(6):
                    copy_layer_name = f"to_q_mv_copy_{i}" if i > 0 else "to_q_mv"
                    if self.use_lora != False:
                ...
                for i in range(6):
                    copy_layer_name = f"to_q_mv_copy_{i}" if i > 0 else "to_q_mv"
                    if self.use_lora != False:
                        part = hidden_states[6:7]
                        if i==0:
                            parts.append(getattr(self, copy_layer_name)(part)+0.5*self.lora_q_B[i](self.lora_q_A(part)))
                        else:
                            parts.append(getattr(self, copy_layer_name)(hidden_states[i+6:i+7])+0.5*self.lora_q_B[i](self.lora_q_A(part)))
                    else:
                        parts.append(getattr(self, copy_layer_name)(hidden_states[i+6:i+7]))
            query_mv = torch.cat(parts, dim=0)
        ...
        if use_mv:
            ...
            if batches == 1:
                for i in range(6):
                    copy_layer_name = f"to_k_mv_copy_{i}" if i > 0 else "to_k_mv"
                    if self.use_lora != False:
                        part = hidden_states[0:1]
                        if i==0:
                            parts.append(getattr(self, copy_layer_name)(part)+0.5*self.lora_k_B[i](self.lora_k_A(part)))
                        else:
                            parts.append(getattr(self, copy_layer_name)(hidden_states[i:i+1])+0.5*self.lora_k_B[i](self.lora_k_A(part)))
                    else:
                        parts.append(getattr(self, copy_layer_name)(hidden_states[i:i+1]))
            else:
                for i in range(6):
                ...
                for i in range(6):
                    ...
                    if self.use_lora != False:
                        ...
                        if i==0:
                            ...
                            parts.append(getattr(self, copy_layer_name)(hidden_states[i+6:i+7])+0.5*self.lora_k_B[i](self.lora_k_A(part)))
                    else:
                        parts.append(getattr(self, copy_layer_name)(hidden_states[i+6:i+7]))
            key_mv = torch.cat(parts, dim=0)
            
            batches = hidden_states.size(0) // 6
            parts = []
            if batches == 1:
                for i in range(6):
                    copy_layer_name = f"to_v_mv_copy_{i}" if i > 0 else "to_v_mv"
                    if self.use_lora != False:
                        part = hidden_states[0:1]
                        if i==0:
                            parts.append(getattr(self, copy_layer_name)(part)+0.5*self.lora_v_B[i](self.lora_v_A(part)))
                        else:
                            parts.append(getattr(self, copy_layer_name)(hidden_states[i:i+1])+0.5*self.lora_v_B[i](self.lora_v_A(part)))
                    else:
                        parts.append(getattr(self, copy_layer_name)(hidden_states[i:i+1]))
            else:
                for i in range(6):
                ...
                for i in range(6):
                    ...
                    if self.use_lora != False:
                        ...
                        if i==0:
                            ...
                            parts.append(getattr(self, copy_layer_name)(hidden_states[i+6:i+7])+0.5*self.lora_v_B[i](self.lora_v_A(part)))
                    else:
                        parts.append(getattr(self, copy_layer_name)(hidden_states[i+6:i+7]))
            value_mv = torch.cat(parts, dim=0)
