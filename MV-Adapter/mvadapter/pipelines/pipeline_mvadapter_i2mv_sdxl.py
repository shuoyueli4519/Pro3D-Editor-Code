...
class MVAdapterI2MVSDXLPipeline(StableDiffusionXLPipeline, CustomAdapterMixin):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        no_noise = kwargs.pop("no_noise", False)
        if no_noise != False:
            ...
            reference_img_tmp = np.array(reference_image)
            first_noise = transform(reference_img_tmp.astype(np.float32) / 255.0)
            first_noise = self.vae.encode(first_noise.unsqueeze(0).to(dtype=torch.float16,device=self.vae.device)).latent_dist.sample()
            first_noise = first_noise * self.vae.config.scaling_factor
            noise_tmp = torch.zeros_like(first_noise)
            timesteps_tmp = torch.tensor([0])
            first_noise = self.scheduler.add_noise(first_noise, noise_tmp, timesteps_tmp)
            print(f"first_noise : {first_noise.shape}")
            print(f"latents : {latents.shape}")
            latents[0] = first_noise

        # Preprocess reference image
        reference_image = self.image_processor.preprocess(reference_image, width=768, height=768)
    ...
    def _copy_mv_linear_adapter(
        self
    ):
        target_keys = ['to_q_mv', 'to_k_mv', 'to_v_mv']
        targets = []
        ...
        for name, module in targets:
            *parent_path, attr_name = name.split('.')
            ...
            for i in range(1, 6):
                copy_name = f"{attr_name}_copy_{i}"
                copy_module = copy.deepcopy(module)
                setattr(parent, copy_name, copy_module)    
                
            original_copy_name = f"{attr_name}_copy_0"
            setattr(parent, original_copy_name, module)        
    ...
    def train_forward(
        ...
        reference_image: Optional[PipelineImageInput] = None,
        reference_conditioning_scale: Optional[float] = 1.0,
        accelerate : Optional[Accelerator] = None,
        **kwargs,
    ):
        # 0. Default height and width to unet
        height = height or self.default_sample_size * self.vae_scale_factor
        ...
        # print(f"reference_latents : {reference_latents}")
