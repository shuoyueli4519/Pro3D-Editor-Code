...
def readSelfdefineCameras(selfdefine_frames, images_folder):
    cam_infos = []
    for frame in selfdefine_frames:
        id = frame['id']
...
def readSelfdefineSceneInfo(path, images, eval, extra_opts=None):
    images_folder = osp.join(path, "images")
    with open(osp.join(path, f"selfdefine.json")) as f:
        selfdefine_frames = json.load(f)
    train_cam_infos = readSelfdefineCameras(
        selfdefine_frames, images_folder
    )
...
sceneLoadTypeCallbacks = {
    "Selfdefine": readSelfdefineSceneInfo,
