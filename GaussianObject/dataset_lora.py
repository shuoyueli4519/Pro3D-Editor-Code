...
class GSCacheDataset(Dataset):
    def __init__(
        ...
        # self.iter = max([int(iter.split('_')[-1]) for iter in os.listdir(os.path.join(self.loo_dir, 'leave_0', 'point_cloud'))
        #                  if os.path.isdir(os.path.join(self.loo_dir, 'leave_0', 'point_cloud', iter)) and iter.split('_')[-1].isdigit()])

        # ply_path = os.path.join(self.data_dir, 'dust3r_4.ply')
        self.ply_path = []
        for i in range(sparse_num):
            self.ply_path.append(os.path.join(self.loo_dir, f"leave_{i}", 'point_cloud', 'iteration_10000', 'point_cloud.ply'))
        self.gaussian = GaussianModel(sh_degree=sh_degree)
        self.gaussian.load_ply(self.ply_path[0], False)
    ...
    def __getitem__(self, idx):
        ...
        self.gaussian.load_ply(self.ply_path[idx], False)
        # print(f"train: {self.train}; manual_noise_prob: {self.manual_noise_prob}")
        if not self.train:
            ...
            print(f"statistics_info: {len(self.statistics_info)}")
            self.gaussian.add_statistics_noise(self.statistics_info, noise_dropout, noise_scale, idx)
            render_pkg = render(cam, self.gaussian, self.pipe, self.bg_color)
            noisy = render_pkg['render']
